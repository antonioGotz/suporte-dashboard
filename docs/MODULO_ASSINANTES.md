...
- Botão “Suspender/Reativar/Cancelar”: Ações administrativas sobre o status do assinante.
  - Ao Suspender/Cancelar, abre-se um modal com seleção de motivo (select) e opção "Outros" que revela um campo de texto complementar. O motivo é enviado ao backend no campo `reason` e persistido em `actions_log.reason`, aparecendo no Histórico.
...
- `SearchBar` (busca)
- `ConfirmationModalWithReason` (modal genérico de confirmação com motivos para suspender/cancelar)
...
- Atualização de status via PUT (`/api/admin/subscribers/{orderId}/status`) com payload `{ status, action, reason }`
...
- Atualização de status consome `/api/admin/subscribers/{orderId}/status` e grava `actions_log.reason`.
...
// Arquivo movido para docs/oficiais/MODULO_ASSINANTES.md
